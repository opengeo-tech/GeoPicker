...
3. the source code is copied last, followed by a second `npm install --ignore-scripts`: npm creates the bin symlink only when the target file exists, and `cli/bin/geopicker` was not present during the first manifest-only install — this re-run is a fast local relink that does not re-trigger native builds;
4. the bash completion script generated by `geopicker bash-completion` (see [CLI](cli.md)) is installed in `/etc/bash_completion.d/geopicker` and sourced by interactive shells, so tab completion works out of the box in `docker exec -it <container> bash`;
5. the container starts the server through the CLI itself: `CMD ["geopicker", "server-start", "--config", "server/config.yml"]` — the path is relative to the `/home` workdir, so it resolves to `/home/server/config.yml`, the same file replaced by the `custom.config.yml` volume mount in `docker-compose.yml`.
