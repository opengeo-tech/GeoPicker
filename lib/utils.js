 ...
 * @return {[type]}         [description]
 */
function datasetInfo(dataset, rasterband) {
  ...
  const {geoTransform} = dataset
      , pixelSize = {
        //TODO convert in meters
        x: geoTransform[1],
        y: -geoTransform[5]
      }

  const {/*unitType: unit,*/ noDataValue: noData, dataType} = rasterband;

  //console.log('GPICKER datasetInfo')
  //console.log(dataset.srs.toWKT(), dataset)
