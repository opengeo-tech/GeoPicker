...
/**
 * generate contour lines for a given value
 ...
 */
async function contourLines(dataset, loc) {
 // use: https://mmomtchev.github.io/node-gdal-async/#contourgenerateasync

  const value = dataset.locPixel(loc);
  
  const layer = gdal.open('temp', 'w', 'Memory')
  const lyr = layer.layers.create('temp', null, gdal.LineString);

  gdal.contourGenerate({
    src: dataset.rasterband,
    dst: lyr,
    fixedLevels: [value]
  });
