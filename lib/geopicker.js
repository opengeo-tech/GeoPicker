...
function openFile(fileData, band = 1) {
    ...
    //TODO getAsync
        ...
        , geoTransform = new gdal.CoordinateTransformation(crs, dataset);
    ...
    return {
        ...
        locPixel: loc => {
            try {
                const { x, y } = geoTransform.transformPoint(Number(loc[0]), Number(loc[1]))
 ...
 */
function getValue(data, dataset, opts = {}) {
    const { band = 1 } = opts
        , { locPixel } = dataset.locPixel
            ? dataset
            : openFile(dataset, band)
 ...
 */
function setValue(data, dataset, opts = {}) {
    const { band = 1 } = opts
        , { locPixel } = dataset.locPixel
            ? dataset
            : openFile(dataset, band)
